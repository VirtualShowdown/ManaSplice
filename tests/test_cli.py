...
def test_paradigm_oop_restructures_top_level_functions_with_wrappers(tmp_path: Path, capsys) -> None:
    ...
    assert exit_code == 0
    updated = source.read_text(encoding="utf-8")
    assert "# Auto-generated by ManaSplice: OOP class" in updated
...
def test_paradigm_semantic_oop_builds_instance_class_with_state(tmp_path: Path, capsys) -> None:
    ...
    assert exit_code == 0
    updated = source.read_text(encoding="utf-8")
    assert "# Auto-generated by ManaSplice: semantic OOP class" in updated
    assert "class PriceCalculator:" in updated
    assert "@staticmethod" not in updated
    assert "def __init__(self, tax_rate=TAX_RATE):" in updated
    assert "self.tax_rate = tax_rate" in updated
    assert "def subtotal(self, items):" in updated
    assert "def total(self, items):" in updated
    assert "return self.subtotal(items) * (1 + self.tax_rate)" in updated
    assert "_DEFAULT_PRICE_CALCULATOR = PriceCalculator()" in updated
    assert "def total(items):\n    return _DEFAULT_PRICE_CALCULATOR.total(items)" in updated
    assert "Restructured 2 function(s) for OOP." in capsys.readouterr().out

    sys.path.insert(0, str(tmp_path))
    try:
        import pricing

        assert pricing.total([10, 5]) == 18
        assert pricing.PriceCalculator(tax_rate=0.5).total([10, 5]) == 22.5
    finally:
        sys.path.remove(str(tmp_path))
...
def test_paradigm_semantic_oop_skips_global_mutation(tmp_path: Path, capsys) -> None:
    ...
    original = (
        "TAX_RATE = 0.2\n\n"
        "def set_tax_rate(value):\n"
        "    global TAX_RATE\n"
        "    TAX_RATE = value\n"
    )
    source.write_text(original, encoding="utf-8")

    exit_code = main(["paradigm", "OOP", "pricing.py", "--cwd", str(tmp_path), "--semantic", "--validate"])

    assert exit_code == 0
    assert source.read_text(encoding="utf-8") == original
    output = capsys.readouterr().out
    assert "functions with global statements are left unchanged" in output
    assert "Restructured 0 function(s) for OOP." in output


def test_paradigm_semantic_rejects_non_oop_styles(tmp_path: Path, capsys) -> None:
    source = tmp_path / "main.py"
    source.write_text("def double(value):\n    return value * 2\n", encoding="utf-8")

    exit_code = main(["paradigm", "functional", "main.py", "--cwd", str(tmp_path), "--semantic"])
...
def test_paradigm_oop_defaults_to_recursive_project_directory(tmp_path: Path) -> None:
    ...
    assert exit_code == 0
    assert 'target_paradigm = "OOP"' in (tmp_path / "pyproject.toml").read_text(encoding="utf-8")
    assert "semantic_oop = true" in (tmp_path / "pyproject.toml").read_text(encoding="utf-8")
    assert "class ServiceService:" in (package / "service.py").read_text(encoding="utf-8")
    assert "class IgnoredOperations:" not in (hidden / "ignored.py").read_text(encoding="utf-8")


def test_run_uses_configured_paradigm(tmp_path: Path, capsys) -> None:
    ...
    assert exit_code == 0
    updated = source.read_text(encoding="utf-8")
    assert "class PricingService:" in updated
    assert "def __init__(self, tax_rate=TAX_RATE):" in updated
    assert "return value * (1 + self.tax_rate)" in updated
    assert "Restructured 1 function(s) for OOP." in capsys.readouterr().out


def test_run_requires_configured_paradigm(tmp_path: Path, capsys) -> None:
    source = tmp_path / "main.py"
    ...
    assert exit_code == 1
    assert "No target paradigm configured" in capsys.readouterr().out


def test_ignore_marks_folder_and_run_skips_it(tmp_path: Path) -> None:
    (tmp_path / "pyproject.toml").write_text(
        "[tool.manasplice]\n"
        'target_paradigm = "OOP"\n'
        "semantic_oop = true\n"
...
def test_paradigm_oop_skips_dynamic_code_execution(tmp_path: Path, capsys) -> None:
    source = tmp_path / "main.py"
    original = (
        "def make_type(name):\n"
        "    body = f\"\"\"\\\n"
...
def test_paradigm_audit_reports_without_writing(tmp_path: Path, capsys) -> None:
    source = tmp_path / "main.py"
    original = (
        "def run():\n"
        "    return 1\n\n"
    ...
    exit_code = main(["paradigm", "OOP", "main.py", "--cwd", str(tmp_path), "--audit", "--json"])
    ...
    assert data["results"][0]["changes"][0]["changed"] is True


def test_paradigm_refuses_broad_facade_without_opt_in(tmp_path: Path, capsys) -> None:
    ...
    exit_code = main(["paradigm", "functional", "--dir", "pkg", "--cwd", str(tmp_path), "--recursive"])

    assert exit_code == 1
    assert "FUNCTIONAL_API" not in first.read_text(encoding="utf-8")
    assert "allow-broad-facade" in capsys.readouterr().out


def test_paradigm_allows_broad_facade_with_explicit_opt_in(tmp_path: Path) -> None:
    package = tmp_path / "pkg"
    package.mkdir()
    first = package / "a.py"
    second = package / "b.py"
    ...
    assert exit_code == 0
    assert "FUNCTIONAL_API" in first.read_text(encoding="utf-8")
    assert "FUNCTIONAL_API" in second.read_text(encoding="utf-8")


def test_paradigm_runs_verification_command_after_writing(tmp_path: Path, capsys) -> None:
    source = tmp_path / "main.py"
    source.write_text(
        "def area(r):\n"
        "    return r * r\n\n"
    ...
    exit_code = main(["paradigm", "OOP", "main.py", "--cwd", str(tmp_path), "--verify-command", command])

    assert exit_code == 0
    assert "Running verification command" in capsys.readouterr().out


def test_paradigm_json_verification_output_stays_machine_readable(tmp_path: Path, capsys) -> None:
    source = tmp_path / "main.py"
    source.write_text(
        "def area(r):\n"
        "    return r * r\n",
    ...
    command = f'"{sys.executable}" -c "print(\'verification noise\')"'

    exit_code = main(["paradigm", "OOP", "main.py", "--cwd", str(tmp_path), "--json", "--verify-command", command])
...
def test_paradigm_procedural_flattens_generated_oop_class(tmp_path: Path) -> None:
    ...
    assert "# Auto-generated by ManaSplice: OOP class" not in updated
    assert "# Auto-generated by ManaSplice: procedural compatibility class" in updated
...
def test_paradigm_procedural_ignores_user_owned_operations_class(tmp_path: Path) -> None:
    source = tmp_path / "main.py"
    original = (
    ...
    exit_code = main(["paradigm", "procedural", "main.py", "--cwd", str(tmp_path), "--validate"])

    assert exit_code == 0
    assert source.read_text(encoding="utf-8") == original


def test_paradigm_procedural_repeat_is_idempotent(tmp_path: Path, capsys) -> None:
    source = tmp_path / "main.py"
    source.write_text("def area(r):\n    return r * r\n", encoding="utf-8")

    assert main(["paradigm", "OOP", "main.py", "--cwd", str(tmp_path), "--validate"]) == 0
    assert main(["paradigm", "procedural", "main.py", "--cwd", str(tmp_path), "--validate"]) == 0
    capsys.readouterr()
    repeat_exit = main(["paradigm", "procedural", "main.py", "--cwd", str(tmp_path), "--validate"])

    assert repeat_exit == 0
    assert "Restructured 0 function(s) for procedural." in capsys.readouterr().out
